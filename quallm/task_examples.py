...
3.  Response Model (Pydantic model class, enforced with the Instructor library at runtime): A Pydantic model class that defines the structure, field names, and data types of the expected output from the LLM performing the actual task. For your analysis, this model's schema is shown to you as a Python dictionary representation generated by calling the `model_json_schema` method of Pydantic's `BaseModel` class. This dictionary uses Python syntax (with single quotes) but represents the exact schema structure that Instructor will use internally (so you can assume that it is an exact representation of the response model).
...
- Response Model Effectiveness: Could the response model be improved for better LLM performance or downstream processing (e.g., changing a string to an enum / literal where appropriate)? Is the response model robust to foreseeable edge cases (e.g., empty or outlying observations, expecting one of something but getting multiple, etc.)? Is the setup likely to result in errors or outputs that are technically valid but practically invalid when encountering foreseeable edge cases? Are there simplifications that would make the task easier (faster, less error-prone) for the LLM without losing information (e.g., avoiding complicated nested structures, many attributes, or rigid constraints, especially if using a smaller model)? Ensure that all suggestions are consistent with Pydantic's capabilities and best practices, as well as the capabilities of the LLM performing the task. Ensure that any suggestions are clear regarding the exact changes to be made (e.g., whether to change an attribute name, type, description, or other aspect) on the assumption that the user may not be deeply familiar with how Pydantic works.
- Placeholders: Identify if the context surrounding placeholders inside curly braces is clear and appropriate for their intended dynamic content. Are there any missing placeholders implied by the task description? Do any placeholders need additional context surrounding them (or more detailed description within the task instructions) for correct interpretation by the LLM performing the task? Is the formatting of placeholders robust to reasonably forseeable variation in the content inserted into them? For instance, if it's likely that instances of the content inserted into the placeholder will span multiple lines, is the prompt formatted such that this will not confuse the LLM performing the task (e.g., by enclosing the placeholder in XML tags, or by using special delimiters such as triple backticks)? (Note that the name of the placeholder is largely irrelevant to the LLM performing the actual task, because these will always be filled with the correct data at runtime, and so the LLM does not see the actual placeholders at runtime.)
- Other Details: Are there any other pertinent suggestions that could enhance the robustness or clarity of this task definition for an LLM? Based on your careful consideration of the available information, draw on your knowledge of appropriate domain(s) including but not limited to you knowledge of (1) the subject matter of the task, (2) relevant best practices from data labelling, psychometrics and/or qualitative research, (3) LLMs, prompt engineering, LLM capabilities and limitations, and (4) the likely intended downstream use of the task's output.
...
- Ensure your feedback clearly points to the specific locations in the prompts or response model model definition where your suggestions apply (e.g., quoting relevant prompt excerpts or directly referring to specific fields in the response model by name) so that the user can efficiently locate and review the components of the task definition you are referring to. Also ensure that your feedback provides a clear indication of priority, for example by distinguishing what is "feedback to consider" vs. "significant issues that need to be addressed".
...
- On user goals and user-provided context:
    ...
    - If the user's goal(s) are not explicitly stated, carefully consider what the their goals are likely to be (e.g., in terms of level of detail, specificity vs. generality / etc. of their intended outputs) before formulating your feedback.
    ...
    - Consider offering suggestions as to what additional contextual details (including but not limited to: user goals and priorities, task context, dataset description, prior pre-processing steps, intended downstream uses, choice of LLM for task, etc.) the user might be able to supply as contextual information (within what you see as the `user_provided_context` tags, which the user supplies to the `feedback` method via the `context` argument) if they were to re-run the feedback process. When offering suggestions as to what additional contextual details the user might supply, ensure your suggestions are limited to the most important or valuable details the user might supply, given all presently available information, and explain the importance of any such details.
